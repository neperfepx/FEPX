...
subroutine crys_initparams(crys)
  ...
  do iphase = 1, size(crys)
    ...
    if (crys(iphase)%precipitation) then
      ...
      bowing_precip = (crys(iphase)%c_p*crys(iphase)%bulk_shear_mod*crys(iphase)%b_m)/(l_precip - (2*crys(iphase)%r_p))
      ...
      if (bowing_precip .ge. cutting_precip) then
        crys(iphase)%precipitation_strength = cutting_precip
      else if (bowing_precip .lt. cutting_precip) then
        crys(iphase)%precipitation_strength = bowing_precip
      end if
      ...
      l_precip = crys(iphase)%r_p*sqrt(pi/crys(iphase)%f_p)
      crys(iphase)%precipitation_strength = &
        & (crys(iphase)%c_p*crys(iphase)%bulk_shear_mod*crys(iphase)%b_m)/(l_precip - (2*crys(iphase)%r_p))
    end if
      ...
      select case (crys(iphase)%structure)
        ...
        case ("bcc")
          call set_bcc_block_matrices(&
            ...
            & crys(iphase)%interaction_matrix_parameters(11), &
            & crys(iphase)%interaction_matrix_parameters(12), &
            & crys(iphase)%interaction_matrix_parameters(13), crys(iphase))
  ...
  subroutine crys_set_default (crys)
    ...
    allocate(crys%interaction_matrix_parameters(13))
    ...
    crys%b_m = -1.0d0
    crys%c_p = -1.0d0
    crys%precipitation_strength = 0.0d0
