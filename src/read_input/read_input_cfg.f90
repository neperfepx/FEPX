...
subroutine read_config(mesh_file, opt_file, ori_file, phase_file, printing, &
    ...
    character(len=20), intent(out) :: opt_file
    ...
    character(len=10000) :: flag
    character(len=10000) :: value
    integer :: file_id
    integer :: j
    integer :: j0
    integer :: state
    ...
    do ! loop over file
      ...
      else if (flag .eq. "read_from_opt_file") then
        opt_file = "simulation.opt"      
  ...
  ! Check for crystal parameter options
        ...
        if (trim(value) .eq. "from_file") then
          mesh%g_0_from_file = .true.
          opt_file = "simulation.opt"
        else
          call parse_g_0(value, crys(phase))
        end if
      else if (flag .eq. "g_0_bcc_112") then 
        crys(phase)%g_0_112 = .true.
        if (trim(value) .eq. "from_file") then
          opt_file = "simulation.opt"
        else
  ...
  subroutine parse_set_bc(value, loading_options)
    ...
    if (bc_var .eq. "vel") then
      ...
      else if (bc_subvar .eq. "grip") then
  ...
  subroutine parse_g_0(value, my_crys)
    ...
    if ((my_crys%structure .eq. "fcc") .or. &
      ...
      if (allocated(my_crys%g_0_tmp) .eqv. .false.) then
        allocate(my_crys%g_0_tmp(1:12))
      end if
      my_crys%num_vals = nvals
      ! Based on number of values input, proceed accordingly
      select case (nvals)
  
        ! Isotropic g_0 (1 value input)
        case(1)
          read(value, *, iostat=ioerr) tmp_crss
          if ((ioerr .ne. 0) ) then
            call par_quit("Error  : parse_g_0 failed.")
          end if
          my_crys%g_0_tmp(1:12) = tmp_crss
    ...
    ! If hcp, try to read 3 or 18 values
      ...
      select case (nvals)
        ...
        case(3)
          ...
          my_crys%g_0_tmp(1:3)= g_0_hcp(1)
          my_crys%g_0_tmp(4:6)= g_0_hcp(2)
          my_crys%g_0_tmp(7:18)= g_0_hcp(3)
    ...
    ! If bct, try to read 10 or 32 values
      ...
      select case (nvals)
        ...
        case(10)
          ...
          my_crys%g_0_tmp(1:2)   = g_0_bct(1)
          my_crys%g_0_tmp(3:4)   = g_0_bct(2)
          my_crys%g_0_tmp(5:6)   = g_0_bct(3)
          my_crys%g_0_tmp(7:10)  = g_0_bct(4)
          my_crys%g_0_tmp(11:12) = g_0_bct(5)
          my_crys%g_0_tmp(13:16) = g_0_bct(6)
          my_crys%g_0_tmp(17:18) = g_0_bct(7)
          my_crys%g_0_tmp(19:20) = g_0_bct(8)
          my_crys%g_0_tmp(21:24) = g_0_bct(9)
          my_crys%g_0_tmp(25:32) = g_0_bct(10)
  ...
  subroutine parse_g_0_bcc_112(value, my_crys)
    ...
    if (my_crys%structure .eq. "bcc") then
      ...
      i = 1
      nvals = count((/(value(i:i), i=1, len_trim(value))/) .eq. " ") + 1
      my_crys%num_g_0_vals_112  = nvals
      if (allocated(my_crys%g_0_tmp) .eqv. .true.) then
        g_0_cubic = my_crys%g_0_tmp(1:12)
        deallocate(my_crys%g_0_tmp)
        allocate(my_crys%g_0_tmp(1:24))
        my_crys%g_0_tmp(1:12) =g_0_cubic(1:12)
      else
        allocate(my_crys%g_0_tmp(1:24))
      end if
      ! Based on number of values input, proceed accordingly
      select case (nvals)
        ...
        ! Isotropic g_0 (1 value input)
        case(1)
          read(value, *, iostat=ioerr) g_0
          if ((ioerr .ne. 0) .or. (g_0 .le. 0.0d0)) then
            call par_quit("Error  : parse_g_0_112 failed.")
          end if
          my_crys%g_0_tmp(13:24) = g_0
        ! Anisotropic g_0 (12 values input)
        case(12)
          read(value, *, iostat=ioerr) my_crys%g_0_tmp(13:24)
          if ((ioerr .ne. 0) .or. (any(my_crys%g_0_tmp .le. 0.0d0))) then
            call par_quit("Error  : parse_g_0_112 failed.")
          end if 
  ...
  subroutine parse_interaction_matrix_parameters(value, my_crys)
    ...
    ! If bcc, try to read 2 or 7 values
      ...
      select case (nvals)
        ...
        ! Fully anisotropic interaction  with 112 slip systems available (3 values input)
        case (3)
          read(value, *, iostat=ioerr) my_crys%interaction_matrix_parameters(1:2),&
          & my_crys%interaction_matrix_parameters(8)
          if ((ioerr .ne. 0) .or. (any(my_crys%interaction_matrix_parameters(1:3) .le. 0.0d0))) then
            call par_quit("Error  : parse_interaction_matrix_parameters failed.")
          end if
        ...
        ! Co-planar interaction with 112 slip systems available (13 values input)
        case (13)
          read (value, *, iostat=ioerr) my_crys%interaction_matrix_parameters(1:13)
          if ((ioerr .ne. 0) .or. (any(my_crys%interaction_matrix_parameters(1:13) .le. 0.0d0))) then
            call par_quit("Error  : parse_interaction_matrix_parameters failed.")
          end if
  ...
  subroutine print_cfg_terminal(my_crys, number_of_phases)
    ...
    if (myid .eq. 0) then
      ...
      if ((my_crys(iphase)%structure .eq. "fcc") .or. &
          ...
          & (my_crys(iphase)%g_0_112 .eqv. .false.))) then

        if (my_crys(iphase)%num_vals .eq. 1) then
          write (*, '(a,e14.6)') 'Info   :     > g_0:           ', &
            & my_crys(iphase)%g_0_tmp(1)
        else if (my_crys(iphase)%num_vals .eq. -1) then
          write (*, '(a)') 'Info   :     > g_0:              from_file'
        else 
          do i = 1, 12
            write (*, '(a,i0,a,e14.6)') 'Info   :     > g_0 (', i, '):       ', &
              & my_crys(iphase)%g_0_tmp(i)
          end do
        end if

      else if ((my_crys(iphase)%structure .eq. "bcc") .and. &
          & (my_crys(iphase)%g_0_112 .eqv. .true.)) then
        if (my_crys(iphase)%num_vals .eq. 1) then
          write (*, '(a,e14.6)') 'Info   :     > g_0:            ', &
            & my_crys(iphase)%g_0_tmp(1)
        else if (my_crys(iphase)%num_vals .eq. -1) then
        ...
        if (my_crys(iphase)%num_g_0_vals_112 .eq. 1) then
          write (*, '(a,e14.6)') 'Info   :     > &
              &g_0_112:       ', my_crys(iphase)%g_0_tmp(13)
        else if (my_crys(iphase)%num_g_0_vals_112 .eq. -1) then
          write (*, '(a)') 'Info   :     > g_0_112:       from_file'
        else
          do i = 1, 9
            write (*, '(a,i0,a,e14.6)') 'Info   :     > &
                &g_0_112 (', i, '):   ',  &
                & my_crys(iphase)%g_0_tmp(12+i)
          end do
          do i = 10, 12
            write (*, '(a,i0,a,e14.6)') 'Info   :     > &
                &g_0_112 (', i, '):  ',  &
        ...
        if (my_crys(iphase)%num_vals .eq. 3) then
          write (*, '(a,e14.6)') 'Info   :     > &
              &g_0 (Basal):   ', &
              & my_crys(iphase)%g_0_tmp(1)
          write (*, '(a,e14.6)') 'Info   :     > &
              &g_0 (Pris.):   ', &
              & my_crys(iphase)%g_0_tmp(4)
          write (*, '(a,e14.6)') 'Info   :     > &
              &g_0 (Pyr.):    ', &
              & my_crys(iphase)%g_0_tmp(7)
        else if (my_crys(iphase)%num_vals .eq. -1) then
          write (*, '(a)') 'Info   :     > g0:              from_file'
        else 
          do i = 1, 18
            write (*, '(a,i0,a,e14.6)') 'Info   :     > &
              &g_0 (', i, '):   ', &
        ...
        if  (my_crys(iphase)%num_vals .eq. 10) then
            write (*, '(a,e14.6)') 'Info   :     > &
              &g_0 (1):       ', &
              & my_crys(iphase)%g_0_tmp(1)                 
            write (*, '(a,e14.6)') 'Info   :     > &
              &g_0 (2):       ', &
              & my_crys(iphase)%g_0_tmp(3)   
              
            write (*, '(a,e14.6)') 'Info   :     > &
              &g_0 (3):       ', &
              & my_crys(iphase)%g_0_tmp(5)   
            ...
            write (*, '(a,e14.6)') 'Info   :     > &
              &g_0 (5):       ', &
              & my_crys(iphase)%g_0_tmp(11) 
              
            write (*, '(a,e14.6)') 'Info   :     > &
              &g_0 (6):       ', &
              & my_crys(iphase)%g_0_tmp(13) 
              
            write (*, '(a,e14.6)') 'Info   :     > &
              &g_0 (7):       ', &
              & my_crys(iphase)%g_0_tmp(17) 
              
            write (*, '(a,e14.6)') 'Info   :     > &
              &g_0 (8):       ', &
              & my_crys(iphase)%g_0_tmp(19) 
              
            write (*, '(a,e14.6)') 'Info   :     > &
              &g_0 (9):       ', &
              & my_crys(iphase)%g_0_tmp(21) 
              
            write (*, '(a,e14.6)') 'Info   :     > &
              &g_0 (10):      ', &
              & my_crys(iphase)%g_0_tmp(25)
      ...
      if (my_crys(iphase)%g_s0 .eq. -1) then
        write (*, '(a,e14.6)') 'Info   :     > g_s0:            from_file'
      else
